# tung_tung_tung_sahur (easy)
## tung_tung_tung_sahur.py
<img width="428" height="484" alt="image" src="https://github.com/user-attachments/assets/39874a6e-b677-4a60-81bc-0597e3d9f273" />

## Phân tích file
- Trong file **tung_tung_tung_shahur.py** ta thấy $C = m^e$ và cứ $C < N$ là $C$ sẽ tăng gấp đôi và chương trình sẽ in ra "Tung!". Khi $C \ge N$ thì $C$ bị trừ đi một lượng $N$ và chương trình sẽ in ra "Sahur!".
- Điều này có nghĩa là sau cuối chương trình $C_{bđ}$ đã gấp thêm $2^k$ lần với k bằng số lần xuất hiện của "Tung!" và bị trừ đi $N$.
- Ta đảo ngược quá trình lại sẽ thu được $C_{bđ} = (C + N)/2^k$ với $k = 164$.
- Sau khi có $C_{bđ}$ ta dễ dàng tìm ra $m = \sqrt[e]{C_{bđ}}$. Từ đó tìm ra **flag**.

## Python code
```python
e = 3
N = 
C = 

# Tung! xuất hiện 164 lần
C_bđ = (C + N)//(2**164)

from sympy import integer_nthroot
m,b = integer_nthroot(C_bđ,e)

print(bytes.fromhex(hex(m)[2:]).decode())
```
