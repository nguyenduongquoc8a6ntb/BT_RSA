# I'm a big, big e in a big, big world (easy)
## Phân tích file
- Tác giả chỉ cho file **bigger.txt** và một gợi ý từ đề là big e.
- Vì e rất lớn dẫn đến việc d sẽ rất nhỏ so với N. Ta dùng có thể dùng **Wiener's attack** để giải quyết bài này.
- Link bài viết **Wiener's attack**:
## Python code
```python
N = 
e = 
c = 

import math
def wiener_attack(e,N,c):
    # Tính ra k/d của từng tầng
    def d_generator(e, N):
        p1, p2, q1, q2 = 1, 0, 0, 1
        n, d = e, N 
        while d:
            a, rem = divmod(n, d)
            n, d = d, rem
            p1, p2, q1, q2 = a * p1 + p2, p1, a * q1 + q2, q1
            yield [p1, q1]  # [p1] là k (tử số), [q1] là d (mẫu số)

    for k,d in d_generator(e,N):
        if k == 0:
            continue
        else:
            phi_N = (e*d - 1) // k

            # Tính S,P suy ra x² - Sx + P = 0
            S = N - phi_N + 1
            P = N
            delta_sqrt = math.isqrt((S**2) - (4*1*P))

            # Tính p,q từ x1,x2
            p = (S + delta_sqrt)//(2*1)
            q = (S - delta_sqrt)//(2*1)

            # Nếu p.q = N thì d là private key
            if p*q == N:
                m = pow(c,d,N)
                plaintext = bytes.fromhex(hex(m)[2:]).decode()
                return plaintext

print(wiener_attack(e,N,c))
```
